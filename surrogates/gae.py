...
class MoEGrammarAE(nn.Module):
    ...
    def _init_bounds_buffers(self):
        # 55 experts. Index 54 is Padding (Bounds 0.0)
    ...
    def decode(self, z):
        ...
        if is_seq:
            ...
            processed_params = processed_params.view(B, L, 55, 14)
...
def preprocess_encoding(batch_encoding):
    ...
    """
    # 1. Discard Epoch (Index 0)
    genome_flat = batch_encoding[:, 1:] 
    
    # 2. Reshape to Blocks (No Filtering, Zeros Kept)
...
def grammar_loss_function(recon_types, recon_params, target_blocks, alpha_type=1.0, alpha_param=1.0, alpha_reg=0.1):
    ...
    # Loss C: Integer Reg (Mask Padding out?)
    # Padding is index 54. Mask template is zeros for 54.
...
def collapse_to_physical(type_logits, param_stack):
    ...
    If Type 54 (Padding), output ZEROS.
    """
    pred_type_indices = torch.argmax(type_logits, dim=2)
    ...
    one_hot_54 = one_hot_55[:, :, :54]
    
    physical_blocks = torch.cat([one_hot_54, selected_params], dim=2)
    
    # Zero Masking for Padding Class
...
def expand_dataframe_to_blocks(data_df, padding_ratio=0.1):
    """
    Expands dataframe into blocks. Keeps zeros. Balances padding.
    """
    valid_rows = []
    padding_rows = []
    ...
    for idx, row in data_df.iterrows():
        ...
        for layer_idx in range(15):
            ...
            if 'epoch_num' in new_row: del new_row['epoch_num']
...
def encode_full_1021(ae, encoding_1021, device=None):
    """
    Vectorized encoding. Returns 240 latents. No Epoch.
    """
    if device is None:
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    ae.eval()
    with torch.no_grad():
        ...
        # Flatten
        z_flat = z_seq.reshape(batch_size, -1) # (B, 240)
