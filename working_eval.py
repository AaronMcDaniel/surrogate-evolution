        ...
        for k, v in target.items():
            ...
            if isinstance(v, torch.Tensor):
                ...
                if k == 'boxes':
                    ...
                    x2 = x1 + v[:, 2]
                    y2 = y1 + v[:, 3]
                    v = torch.stack((x1, y1, x2, y2), dim=1)
...
def draw_boxes(img, flight_id, frame_id, pred_boxes, true_boxes, outdir='test/outputs'):
...
if __name__ == '__main__':
    ...
    train_one_epoch(model, device, train_loader, optimizer, scheduler, scaler, max_batch=1000)
    print('Done!')
    
    print("Evaluating epoch...")
    val_one_epoch(model, device, val_loader, iou_thresh, conf_thresh, loss_weights, iou_thresh, max_batch=1)
    print("Done!")
