...
def simplify_individual(individual):
    """
    Simplify a DEAP individual by performing constant folding on its tree representation.
    
    Args:
        individual: A DEAP individual (typically a gp.PrimitiveTree or similar)
        
    Returns:
        individual: A new individual with the same type as input but with simplified tree
    """
    from primitive_tree import CustomPrimitiveTree
    
    # Convert individual to string representation
    tree_string = str(individual)
    
    # Simplify the tree string
    simplified_string = simplify_tree(tree_string)
    
    # If no simplification occurred, return the original individual
    if simplified_string == tree_string:
        return individual

    # Use CustomPrimitiveTree to convert back to individual
    # And create a new individual of the same type as the original
    simplified_individual = creator.Individual(CustomPrimitiveTree.from_string(simplified_string, primitives.pset))
    
    # Copy fitness values if they exist
    if hasattr(individual, 'fitness') and individual.fitness.valid:
        simplified_individual.fitness.values = individual.fitness.values
