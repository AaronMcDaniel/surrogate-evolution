...
class DetTensor: # acts as an end type
    pass

class DetResultsTensor:
    pass
...
# Detection Heads
def FasterRCNN_Head(tensor: Tensor3D):
    return DetResultsTensor()

def FCOS_Head(tensor: Tensor3D):
    return DetResultsTensor()

def RetinaNet_Head(tensor: Tensor3D):
    return DetResultsTensor()

def SSD_Head(tensor: Tensor3D):
    return DetResultsTensor()

def DetNeuralNetwork(tensor: DetResultsTensor, optimizer: Optimizer, scheduler: Scheduler):
    return DetTensor()

# Classification Heads
def ClsNeuralNetwork(tensor: ClsResultsTensor, optimizer: Optimizer, scheduler: Scheduler):
    return ClsTensor()
...
# These should be pulled from the config
input_tensors = [Tensor3D, Optimizer, Scheduler]
output_tensor = DetTensor

# creating primitive set from layers and components
pset = gp.PrimitiveSetTyped("MAIN", input_tensors, output_tensor, "IN")
...
pset.addPrimitive(DetNeuralNetwork,
                  [DetResultsTensor, Optimizer, Scheduler] + list(itertools.repeat(ProbFloat, num_loss_components)),
                  DetTensor)

# Classification 
pset.addPrimitive(ClsNeuralNetwork,
                  [ClsResultsTensor, Optimizer, Scheduler] + list(itertools.repeat(ProbFloat, num_loss_components)),
                  ClsTensor)
