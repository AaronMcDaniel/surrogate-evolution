...
class Strong_Codec:
    ...
    def encode_surrogate(self, genome, epoch_num):
        if (self.surrogate_encoding_strat.lower() == 'string2vec'):
            ...
            encoded_genome = np.zeros((STRONG_REP_SIZE + num_layer_types, self.max_layers))
    ...
    def construct_head(self, layer_info, num_layer_types):
        ...
        for i, param in enumerate(params):
            if min(params) == max(params):
                layer[num_layer_types + STRONG_INDEX_MAPPING[name][i]] = param / len(params)
            else:
                layer[num_layer_types + STRONG_INDEX_MAPPING[name][i]] = (param - min(params)) / (max(params) - min(params))
